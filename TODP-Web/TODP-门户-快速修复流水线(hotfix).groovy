	...
	if ("${params.Action}" ==~ /1-.*/) {
		waitUntil {
			try {
				emailext (
					body: """
					<p>请开发组长用个人账户登录Jenkins Pipeline页面，同时为前端和后端项目拉取hotfix分支用于线上问题快速修复</p>
					...
					subject: "${env.JOB_NAME}-${env.BUILD_NUMBER}-请开发组长同时为前端和后端项目拉取hotfix分支",
					attachLog: true
				)

                def input_map = input(
				message: '是否同时为前端和后端项目拉取hotfix分支?（仅开发组长有权限执行此步）',
				ok: "同意拉取hotfix分支",
				parameters: [
				...
				emailext (
					body: """
					<p>同时为前端和后端项目拉取Hotfix分支${env.Hotfix_Branch_Name}成功，进入开发快速修复阶段</p>
					...
					subject: "${env.JOB_NAME}-${env.BUILD_NUMBER}-同时为前端和后端项目拉取Hotfix分支成功",
					attachLog: true
				)
				...
				emailext (
					body: """
					<p>同时为前端和后端项目拉取Hotfix分支${env.Hotfix_Branch_Name}失败，可能原因：<p>
					...
					""",
					to: "${env.Dev_Mail_List}",
					subject: "${env.JOB_NAME}-${env.BUILD_NUMBER}-同时为前端和后端项目拉取Hotfix分支失败",
					attachLog: true
				)
	...
	if ("${params.Action}" ==~ /1-.*|2-.*|3-.*|4-.*/) {
		...
		waitUntil {
			...
			try {
				...
				message: "是否同时将前端和后端项目的${env.Hotfix_Branch_Name}分支合并到develop分支?（仅开发组长有权限执行此步）",
				ok: "同意进行分支合并",
				submitter: "${env.Dev_Leader_User}",
				submitterParameter: 'Stage_Submitter'
				)
				...
				emailext (
					body: """
					<p>同时将前端和后端项目的hotfix分支${env.Hotfix_Branch_Name}合并到develop分支失败</p>
					...
					""",
					to: "${env.Dev_Mail_List}",
					subject: "${env.JOB_NAME}-${env.BUILD_NUMBER}任务-同时将前端和后端项目的hotfix分支${env.Hotfix_Branch_Name}合并到develop分支失败",
					attachLog: true
				)
		...
		waitUntil {
			...
			try {
				env.Stage_Submitter = input(
				message: "是否同时将前端和后端项目的hotfix分支${env.Hotfix_Branch_Name}分支合并到master分支?（仅开发组长有权限执行此步）",
				ok: "同意进行分支合并",
				submitter: "${env.Dev_Leader_User}",
				submitterParameter: 'Stage_Submitter'
				)
				...
				emailext (
					body: """
					<p>同时将前端和后端项目的hotfix分支${env.Hotfix_Branch_Name}合并到develop分支和master分支成功</p>
					<p>代码合并触发用户：${env.Stage_Submitter}</p>
					""",
					to: "${env.Dev_Mail_List},${env.QA_Mail_List},${env.PM_Mail_List}",
					subject: "${env.JOB_NAME}-${env.BUILD_NUMBER}-同时将前端和后端项目的hotfix分支${env.Hotfix_Branch_Name}合并到develop分支和master分支成功",
					attachLog: true
				)
				true
			}
				...
				emailext (
					body: """
					<p>同时将前端和后端项目的hotfix分支${env.Hotfix_Branch_Name}合并到master分支失败</p>
					<p>可能合并发生冲突，请开发本地手动执行分支合并</p>
					<p>冲突详情可以查看错误步骤日志： <a href='${env.JENKINS_URL}blue/organizations/jenkins/${env.JOB_NAME}/detail/${env.JOB_NAME}/${env.BUILD_NUMBER}/pipeline'>${env.JOB_NAME}步骤日志</a></p>
					<p>手动解决冲突的指令步骤：</p>
					...
					""",
					to: "${env.Dev_Mail_List}",
					subject: "${env.JOB_NAME}-${env.BUILD_NUMBER}任务-同时将前端和后端项目的hotfix分支${env.Hotfix_Branch_Name}合并到master分支失败",
					attachLog: true
				)
		...
		waitUntil {
			...
			try {
                ...
                def input_map = input(
				message: "前端和后端项目的${env.Hotfix_Branch_Name}已合并到master分支，请同时打上快速修复小版本tag?（仅开发组长有权限执行此步）",
				ok: "同意给master分支打tag",
				parameters: [
				...
				sh '''
				echo "前端项目在master分支打快速修复后的小版本tag"
				cd ${WORKSPACE}
				git checkout master
				git pull origin master
				...
				emailext (
					...
					""",
					to: "${env.Dev_Mail_List}",
					subject: "${env.JOB_NAME}-${env.BUILD_NUMBER}任务-同时在前端和后端项目的master分支打小版本tag - ${env.Minor_Tag_Name}失败",
					attachLog: true
				)
		...
		waitUntil {
			...
			try {
				def input_map = input(
				message: "合并成功，是否同时删除前端和后端项目的${env.Hotfix_Branch_Name}分支?（仅开发组长有权限执行此步）",
				ok: "继续",
				parameters: [
				choice(choices: "Yes\nNo\n", description: "是否同时删除前端和后端项目的${env.Hotfix_Branch_Name}分支!（如确认合并成功，建议删除）", name: 'Hotfix_Branch_Delete')
				],
				submitter: "${env.Dev_Leader_User}",
				submitterParameter: 'Stage_Submitter'
				)
				...
				if ("${env.Hotfix_Branch_Delete}".contains("Yes")) {
					...
					emailext (
						...
						""",
						to: "${env.Dev_Mail_List},${env.QA_Mail_List},${env.PM_Mail_List}",
						subject: "${env.JOB_NAME}-${env.BUILD_NUMBER}-同时删除前端和后端项目的${env.Hotfix_Branch_Name}分支成功",
						attachLog: true
					)
				}			
				true
				...
				emailext (
					body: """
					<p>同时删除前端和后端项目的hotfix分支${env.Hotfix_Branch_Name}失败</p>
					...
					""",
					to: "${env.Dev_Mail_List}",
					subject: "${env.JOB_NAME}-${env.BUILD_NUMBER}-同时删除前端和后端项目的hotfix分支${env.Hotfix_Branch_Name}失败",
					attachLog: true
				)
