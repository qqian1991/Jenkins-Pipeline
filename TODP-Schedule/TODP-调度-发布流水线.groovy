	...
	if ("${params.Action}" ==~ /1-.*/) {
		waitUntil {
			try {
				...
				sh 'echo "${Feature_Branch_Name}" > ../Schedule_Feature_Branch_Name.txt'
...
def Stage2() {
	if ("${params.Action}" ==~ /1-.*|2-.*/) {
		waitUntil {
			try {
				// 读取之前写文件的变量
				def Feature_Branch_Name = readFile '../Schedule_Feature_Branch_Name.txt'
				...
				if (!"${env.Dev_Completion}".contains("Yes")) {
					...
					message: '是否开始部署开发容器环境?（相关开发有权限执行此步）',
					ok: "同意进行开发环境容器部署",
					submitter: "${env.Dev_User_List}",
					...
					def dev_build_step = build (
						...
						string(name: 'Schedule_Deploy_Branch', value: "origin/${env.Feature_Branch_Name}"),
	...
	if ("${params.Action}" ==~ /1-.*|2-.*|3-.*/) {
			...
			try {
				...
				if ("${env.Feature_Branch_Delete}".contains("Yes")) {
					...
					emailext (
						...
						<p>分支删除触发用户：${env.Stage_Submitter}</p>
						""",
						to: "${Dev_Mail_List},${PM_Mail_List},${QA_Mail_List}",
		...
		waitUntil {
			try {
				...
				sh 'echo "${Release_Branch_Name}" > ../Schedule_Release_Branch_Name.txt'
...
def Stage4() {
	if ("${params.Action}" ==~ /1-.*|2-.*|3-.*|4-.*/) {
		waitUntil {
			try {
				// 读取之前写文件的变量
				def Release_Branch_Name = readFile '../Schedule_Release_Branch_Name.txt'
				...
				if (!"${env.QA_Completion}".contains("Yes")) {
					...
					def test_build_step = build (
						...
						string(name: 'Schedule_Deploy', value: "yes"),
						string(name: 'Schedule_Deploy_Branch', value: "origin/${env.Release_Branch_Name}"),
						string(name: 'Web_Frontend_Deploy', value: "yes"),
					...
					emailext (
						body: """
						<p>最新的测试容器环境部署成功</p>
						<p>测试容器环境前端登录地址和端口：10.142.246.4:8888</p>
						<p>测试容器环境后端地址和端口：10.142.246.4:8777</p>
	...
	if ("${params.Action}" ==~ /1-.*|2-.*|3-.*|4-.*|5-.*|6-.*/) {
		waitUntil {
			try {
                ...
                env.Stage_Submitter = input(
				message: '是否使用master分支部署用于冒烟测试的容器环境?（QA有权限执行此步）',
				ok: "同意进行测试环境部署",
				submitter: "${env.QA_User_List}",
				submitterParameter: 'Stage_Submitter'
				)
